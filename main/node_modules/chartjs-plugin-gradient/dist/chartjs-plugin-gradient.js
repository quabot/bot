...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global["chartjs-plugin-gradient"] = factory(global.Chart.helpers, global.Chart));
})(this, (function (helpers, chart_js) { 'use strict';

const isChartV3 = chart_js.Chart.version;
...
function scaleValue(scale, value) {
  const normValue = helpers.isNumber(value) ? parseFloat(value) : parse(scale, value);
...
const legendBoxHeight = (chart, options) => options.labels && options.labels.font && helpers.defined(options.labels.font.size)
...
function addColors(scale, colors, stopColors) {
  for (const value of Object.keys(colors)) {
    ...
    if (isFinite(pixel) && isFinite(stop)) {
      const colorStop = helpers.color(colors[value]);
