...
async function getWelcomeConfig(client, guildId) {
    ...
    if (!joinConfig)
        joinConfig = await JoinLeave.findOne(
            {
                guildId: guildId,
            },
            (err, joinleave) => {
...
async function getVerifyConfig(client, guildId) {
    ...
    if (!verifyConfig)
        verifyConfig = await Verify.findOne(
            {
                guildId: guildId,
            },
            (err, verify) => {
...
async function getMembersConfig(client, guildId) {
    ...
    if (!membersConfig)
        membersConfig = await Members.findOne(
            {
                guildId: guildId,
            },
            (err, members) => {
...
async function getModerationConfig(client, guildId) {
    ...
    if (client.cache.get(`${guildId}-moderation-config`))
        moderationConfig = client.cache.get(`${guildId}-moderation-config`);
    if (!moderationConfig)
        moderationConfig = await Mod.findOne(
            {
                guildId: guildId,
            },
            (err, members) => {
...
async function getCustomizationConfig(client, guildId) {
    ...
    if (client.cache.get(`${guildId}-customization-config`))
        customizationConfig = client.cache.get(`${guildId}-customization-config`);
    if (!customizationConfig)
        customizationConfig = await Custom.findOne(
            {
                guildId: guildId,
            },
            (err, custom) => {
...
async function getLevelConfig(client, guildId) {
    ...
    if (!levelConfig)
        levelConfig = await Level.findOne(
            {
                guildId: guildId,
            },
            (err, level) => {
...
async function getApplicationConfig(client, guildId) {
    ...
    if (client.cache.get(`${guildId}-application-config`))
        applicationConfig = client.cache.get(`${guildId}-application-config`);
