...
async function endGiveaway(client, document, color) {
    ...
    channel.messages
        ...
        .then(async message => {
            ...
            const reactionsShuffle = await shuffleArray(
                (array = Array.from(
            ...
            if (isWinner === true)
                channel
                    .send({
                        embeds: [await generateEmbed(color, `${winMsg}, you won **${giveaway.prize}**!`)],
                        content: `${winMsg}`,
                    })
                    .catch(e => {});
    ...
    await channel.send({
        embeds: [
            new EmbedBuilder()
                ...
                .setColor('3a5a74')
                .setDescription(
                    `<@${data.user}> voted for QuaBot! They will now have a **1.5x** level xp multiplier for 12 hours.\nVote [here](https://top.gg/bot/995243562134409296/vote).`
                ),
        ],
    });
    ...
    client.users.cache
        ...
        .send({
            embeds: [
                new EmbedBuilder()
                    .setColor('3a5a74')
                    .setDescription('Thanks for voting for QuaBot! You have recieved a **1.5x** level xp boost.'),
            ],
        })
        .catch(e => {});
}
