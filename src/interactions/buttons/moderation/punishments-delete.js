...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (!id) return await interaction.reply({
			embeds: [
				new Embed(color)
					.setDescription('An internal error occurred: no punishment ID in embed footer found.')
			], ephemeral: true
		});
		...
		if (!punishment) return await interaction.reply({
			embeds: [
				new Embed(color)
					.setDescription('An internal error occurred: no punishment found for the ID.')
			], ephemeral: true
		});
		...
		await interaction.followUp({
			embeds: [
				new Embed(color)
					.setDescription('Punishment deleted.')
			], ephemeral: true
		});
