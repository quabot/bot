...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (verifyConfig.verifyEnabled === false)
            return interaction
                .reply({
        ...
        if (verifyConfig.verifyCode) {
            ...
            if (modalInteraction) {
                ...
                await modalInteraction.deferReply({ ephemeral: true }).catch(e => {});
                ...
                verifyConfig.verifyRoles.forEach(role => {
                    if (role.bot && interaction.user.bot) {
                        ...
                        if (fRole) {
                            setTimeout(() => {
                                interaction.member.roles.add(fRole).catch(e => {});
                            }, role.delay);
                        }
                    }
                ...
                if (verifyConfig.verifyLog && logChannel)
                    logChannel
                        .send({
                            embeds: [
                                new EmbedBuilder()
