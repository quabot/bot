...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        await interaction.showModal(colorModal).catch(e => {});
        ...
        if (modal) {
            ...
            if (!enteredColor)
                modal
                    .editReply({
                        embeds: [await generateEmbed(color, 'No color entered, try again.')],
                        ephemeral: true,
                    })
                    .catch(e => {});
            if (!/^#([0-9A-F]{6}){1,2}$/i.test(enteredColor))
                modal
                    .editReply({
                        embeds: [await generateEmbed(color, 'Please enter a valid hex color code.')],
                        ephemeral: true,
                    })
                    .catch(e => {});
