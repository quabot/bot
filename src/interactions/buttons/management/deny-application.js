...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        interaction.message.edit({
            embeds: [
                EmbedBuilder.from(interaction.message.embeds[0]).setColor(Colors.Red).setDescription(`Application ID: ${found.applicationId}\nSubmitted by: <@${found.userId}>\nState: **DENIED**\n\nView the answers [on our dashboard](https://dashboard.quabot.net) (discord support coming soon).`)
            ], components: [
        ...
        if (user) user.send({
            embeds: [await generateEmbed(color, "Your application in " + interaction.guild.name + " was denied! You applied for `" + found.applicationId + "`.")]
        }).catch((e => { }));
