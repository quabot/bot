...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		const url = msg.match(/\b((https?|ftp|file):\/\/|(www|ftp)\.)[-A-Z0-9+&@#%?=~_|$!:,.;]*[A-Z0-9+&@#%=~_|$]/ig)[0];
		...
		if (embed.data.description !== '\u200b') await message.edit({ content: interaction.message.content, embeds: [embed] });
