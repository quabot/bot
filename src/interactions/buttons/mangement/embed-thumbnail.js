...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (modal) {
			...
			if (!thumbnail) return await modal.editReply({
				embeds: [
					new Embed(color)
						.setDescription('No thumbnail entered, try again.')
				],
			});

			if (!isValidHttpUrl(thumbnail)) return await modal.editReply({
				embeds: [
					new Embed(color)
						.setDescription('No valid thumbnail entered, try again.')
				],
			});
			...
			await modal.editReply({
				embeds: [
					new Embed(color)
						.setDescription(`Set the thumbnail to: \n**${thumbnail}**`.slice(0, 2000))
				],
			});
		}
	},
