...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (modal) {
            ...
            if (!image) return await modal.editReply({
                embeds: [
                    new Embed(color)
                        .setDescription('No image entered, try again.')
                ],
            });

            if (!isValidHttpUrl(image)) return await modal.editReply({
                embeds: [
                    new Embed(color)
                        .setDescription('No valid image entered, try again.')
                ],
            });
            ...
            await modal.editReply({
                embeds: [
                    new Embed(color)
                        .setDescription(`Set the image to: \n**${image}**`.slice(0, 2000))
                ],
            });
        }
    },
