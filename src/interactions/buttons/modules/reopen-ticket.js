...
module.exports = {
    ...
    async execute(client, interaction, color) {
            ...
            return interaction
                .editReply({
                    embeds: [
                        await generateEmbed(
                            color,
                            "Couldn't find the Tickets category. Configure this on [our dashboard](https://dashboard.quabot.net)."
                        ),
                    ],
                })
                .catch(e => {});
        ...
        channel.setParent(openCategory, { lockPermissions: false }).catch(e => {});

        channel.permissionOverwrites.edit(ticketFound.owner, { ViewChannel: true, SendMessages: true }).catch(e => {});

        ticketFound.users.forEach(user => {
            channel.permissionOverwrites.edit(user, { ViewChannel: true, SendMessages: true }).catch(e => {});
        });
