const { Client, ButtonInteraction, ColorResolvable, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionFlagsBits, ModalBuilder, TextInputStyle, TextInputBuilder, ChannelType } = require('discord.js');
...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (config.topicButton) {
			...
			if (modalResponse && modalResponse.customId === 'ticket-topic') topic = modalResponse.fields.getTextInputValue('topic');
			if (!modalResponse) return;

			const category = interaction.guild.channels.cache.get(config.openCategory);
			if (!category || category.type !== ChannelType.GuildCategory) return await modalResponse.reply({
				embeds: [
					new Embed(color)
			...
			if (!channel) return await modalResponse.reply({
				embeds: [
					new Embed(color)
		...
		const category = interaction.guild.channels.cache.get(config.openCategory);
		if (!category || category.type !== ChannelType.GuildCategory) return await interaction.reply({
			embeds: [
				new Embed(color)
					.setDescription(`Couldn't find any valid configured categories for tickets. You can do this on our [dashboard](https://quabot.net/dashboard/${interaction.guildId}/modules/tickets).`)
			], ephemeral: true
