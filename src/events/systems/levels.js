    ...
    if (xp + rndXp >= reqXp) {
      ...
      const parse = s => {
        return s
          ...
          .replaceAll('{user.avatar_url}', `${message.author.displayAvatarURL()}` ?? '')
          .replaceAll('{user.avatar}', `${message.author.avatar}` ?? '')
          .replaceAll('{avatar}', `${message.author.displayAvatarURL()}` ?? '')
      ...
      nextCheck.forEach(async check => {
        ...
        if (config.rewardDmType === 'text')
          await message.member.send({ content: `${parseCheck(config.rewardDmMessageText)}`, components: [sentFrom] });
      });
    } else {
      levelDB.xp += rndXp;
      await levelDB.save();
