...
module.exports = {
    ...
    */
    async execute(interaction, client) {
        ...
        if (!serverConfig) return await interaction.editReply({
            embeds: [
                new Embed('#416683')
        ...
        if (!appid) return await interaction.editReply({
            embeds: [
                new Embed(color)
                    .setDescription('The application form couldn\'t be found.')
            ]
        });

        const application = await Application.findOne({ id: appid, guildId: interaction.guildId });
        if (!application) return await interaction.editReply({
            embeds: [
                new Embed(color)
                    .setDescription('The application form couldn\'t be found.')
            ]
        ...
        await interaction.editReply({
            embeds: [
                new Embed(color)
                    .setDescription(`You can apply for **${application.name}** by clicking the button below.`)
            ],
            components: [
