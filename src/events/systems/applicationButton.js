...
module.exports = {
  ...
  async execute(interaction, client) {
      ...
      return await interaction.editReply({
        embeds: [
          new Embed(color).setDescription(
            "The application form couldn't be found.",
          ),
        ],
      });
    ...
    await interaction.editReply({
      embeds: [
        new Embed(color).setDescription(
          `You can apply for **${application.name}** by clicking the button below.`,
        ),
      ],
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            ...
            .setURL(
              `https://quabot.net/dashboard/${interaction.guildId}/user/applications/form/${application.id}`,
            ),
        ),
      ],
    });
