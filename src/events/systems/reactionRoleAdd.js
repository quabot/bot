...
module.exports = {
  ...
  async execute(reaction, user, client) {
    ...
    if (
      reactionRole.reqPermission !== "None" &&
      reactionRole.reqPermission !== "none"
    ) {
    ...
    const customConfig = await getServerConfig(
      client,
    ...
    reactionRole.excludedRoles.forEach((r) => {
      if (reaction.message.member.roles.cache.some((ra) => ra.id === r))
        excluded = true;
    });
    let required = false;
    ...
    const sentFrom = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(
          "Sent from server: " + reaction.message.guild?.name ?? "Unknown",
        )
      ...
      switch (type) {
        ...
        case "unique":
          ...
          reactions.forEach((item) => {
            ...
            if (!ra) return;
            if (ma.roles.cache.some((rd) => rd.id === `${item.roleId}`))
              hasRole = true;
          });
