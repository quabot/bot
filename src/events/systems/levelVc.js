      ...
      (function loop() {
        setTimeout(async function () {
          ...
          if (newState.member.voice.selfMute) {
            ...
            const configColor = await getServerConfig(
              client,
              newState.guild.id,
            );
            ...
            const sentFrom = new ActionRowBuilder().addComponents(
              new ButtonBuilder()
                ...
                .setLabel(
                  "Sent from server: " + newState.guild.name ?? "Unknown",
                )
            ...
            const vote = await Vote.findOne(
              ...
              .catch(() => {});
            if (vote) {
              if (parseInt(vote.lastVote) + 43200000 > new Date().getTime())
                rndXp = rndXp * 1.5;
            }
            ...
            if (xp + rndXp >= reqXp) {
              ...
              const parse = (s) => {
                return s
                  ...
                  .replaceAll(
                    "{user.created_at}",
                    `${newState.member.user.createdAt}` ?? "",
                  )
                  .replaceAll(
                    "{user.joined_at}",
                    `${newState.member.joinedAt}` ?? "",
                  )
              ...
              if (config.dmEnabled) {
                ...
                if (config.dmType === "card") {
                  ...
                  if (card) {
                    ...
                    if (!config.cardMention)
                      await newState.member.send({ files: [attachment] });
                    if (config.cardMention)
                      await newState.member.send({
                        files: [attachment],
                        content: `${newState.member}`,
              ...
              const nextCheck =
                config.rewards.filter((i) => i.level === level) ?? [];
              nextCheck.forEach(async (check) => {
                ...
                if (config.rewardDmType === "embed")
                  await newState.member.send({
                    embeds: [
                      new CustomEmbed(config.rewardDmMessage, parseCheck),
                    ],
