                        ...
                        if (xp + rndXp >= reqXp) {
                            ...
                            const parse = (s) => {
                                return s
                                    ...
                                    .replaceAll('{icon}', `${newState.guild.iconURL()}` ?? '') // Deprecated
                                    .replaceAll('{id}', `${newState.member.id}` ?? '') // Deprecated
                                    ...
                                    .replaceAll('{tag}', `${newState.member.user.tag}` ?? '') // Deprecated
                                    ...
                                    .replaceAll('{avatar}', `${newState.member.user.avatarURL()}` ?? '') // Deprecated
                            ...
                            if (config.channel !== 'none') {
                                let channel = config.channel === 'current' ? newState.channel : newState.guild.channels.cache.get(`${config.channel}`);
                                if (!channel) return;
                                ...
                                if (config.messageType === 'card') {
                                    const card = await drawCard(newState.member, newState.member.user, level, xp, formula(level), config.levelCard);
                            ...
                            if (config.dmEnabled) {
                                ...
                                if (config.dmType === 'embed') await newState.member.send({ embeds: [embed], content: `${parse(config.dmMessage.content)}`, components: [sentFrom] });
                                if (config.dmType === 'text') await newState.member.send({ content: `${parse(config.dmMessageText)}` });
                                if (config.dmType === 'card') {
                                    const card = await drawCard(newState.member, newState.member.user, level, xp, formula(level), config.levelCard);
                                    if (!card) await newState.member.send('You leveled up! Sorry, we tried to send a card to the configured channel, but there was an error. Sorry for the inconvinience! All level rewards have been given.');

                                    if (card) {
                                        const attachment = new AttachmentBuilder(card, { name: 'level_card.png' });

                                        if (!config.cardMention) await newState.member.send({ files: [attachment] });
                                        if (config.cardMention) await newState.member.send({ files: [attachment], content: `${message.author}` });
                                    }
                                }
                            }
                            ...
                            nextCheck.forEach(async check => {
                                ...
                                if (config.rewardDmType === 'embed') await newState.member.send({ embeds: [new CustomEmbed(config.rewardDmMessage, parseCheck)], content: `${parseCheck(config.rewardDmMessage.content)}`, components: [sentFrom] });
                                if (config.rewardDmType === 'text') await newState.member.send({ content: `${parseCheck(config.rewardDmMessageText)}`, components: [sentFrom] });
                            });
