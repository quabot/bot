					...
					if (newState.member.voice.selfMute) {
						...
						if (xp + rndXp >= reqXp) {
							...
							if (config.channel !== 'none') {
								const channel = config.channel === 'current' ? newState.channel : newState.guild.channels.cache.get(`${config.channel}`);
								if (!channel) return;
							...
							if (config.dmEnabled) {
								...
								if (config.dmType === 'card') {
									...
									if (card) {
										...
										if (config.cardMention) await newState.member.send({ files: [attachment], content: `${newState.member}` });
									}
								}
							}
