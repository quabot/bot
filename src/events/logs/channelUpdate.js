...
module.exports = {
    ...
    async execute(oldChannel, newChannel, client) {
        ...
        if (oldChannel.type !== newChannel.type) description += `\n**Type:**\n\`${channelTypesById[oldChannel.type]}\` -> \`${channelTypesById[newChannel.type]}\``;
        if (oldChannel.name !== newChannel.name) description += `\n**Name:** \n\`${oldChannel.name}\` -> \`${newChannel.name}\``;
        if (oldChannel.topic !== newChannel.topic) description += `\n**Description:** \n\`${oldChannel.topic ? `${oldChannel.topic}` : 'None'}\` -> \`${newChannel.topic ? `${newChannel.topic}` : 'None'}\``;
        if (oldChannel.parentId !== newChannel.parentId) description += `\n**Category:** \n${oldChannel.parentId ? `<#${oldChannel.parentId}>` : 'none'} -> ${newChannel.parentId ? `<#${newChannel.parentId}>` : 'none'}`;
        if (oldChannel.nsfw !== newChannel.nsfw) description += `\n**NSFW:** \n\`${oldChannel.nsfw ? `Yes` : 'No'}\` -> \`${newChannel.nsfw ? `Yes` : 'No'}\``;
        if (oldChannel.rateLimitPerUser !== newChannel.rateLimitPerUser) description += `\n**Ratelimit:** \n\`${oldChannel.rateLimitPerUser}s\` -> \`${newChannel.rateLimitPerUser}s\``;
        if (oldChannel.rtcRegion !== newChannel.rtcRegion) description += `\n**Region:** \n\`${oldChannel.rtcRegion ? `${oldChannel.rtcRegion}` : 'Automatic'}\` -> \`${newChannel.rtcRegion ? `${newChannel.rtcRegion}` : 'Automatic'}\``;
        if (oldChannel.bitrate !== newChannel.bitrate) description += `\n**Bitrate:** \n\`${oldChannel.bitrate / 1000}kbps\` -> \`${newChannel.bitrate / 1000}kbps\``;
        if (oldChannel.userLimit !== newChannel.userLimit) description += `\n**User Limit:** \n\`${oldChannel.userLimit}\` -> \`${newChannel.userLimit}\``;
        if (oldChannel.defaultAutoArchiveDuration !== newChannel.defaultAutoArchiveDuration) description += `\n**Auto Archive:** \n\`${oldChannel.defaultAutoArchiveDuration}s\` -> \`${newChannel.defaultAutoArchiveDuration}s\``;
