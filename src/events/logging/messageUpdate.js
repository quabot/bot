...
module.exports = {
    ...
    async execute(oldMessage, newMessage, client, color) {
        ...
        if (!logConfig) return;
            ...
            if (logConfig.logExcludedChannels && logConfig.logExcludedChannels.includes(newMessage.channel.id)) return;
            if (logConfig.logExcludedCategories && newMessage.channel.parentId && logConfig.logExcludedCategories.includes(newMessage.channel.parentId)) return;
        ...
        if (
            ...
            (Oldcontent.content === '' && oldMessage.attachments === null && newMessage.attachments === null)
        ) {
            return;
        } else {
        ...
        if (
            ...
            (Newcontent.content === '' && newMessage.attachments === null && oldMessage.attachments === null)
        ) {
            return;
        } else {
            if (Newcontent.content !== null || Newcontent.content !== '') {
        ...
        if (newMessage.author === null || newMessage.author === '' || newMessage.author.avatar === null) {
        } else {
            embed.setFooter({
                text: `User: ${newMessage.author.tag}`,
                iconURL: `${newMessage.author.avatarURL({ dynamic: true })}`,
            });
        }
