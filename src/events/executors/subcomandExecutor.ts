...
module.exports = {
    ...
    async execute(interaction: Interaction, client: Client) {
            ...
            return await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor(Colors.Red)
                        .setDescription(
                            `⚠️ An error occurred! Couldn't find the command ${
                                interaction.commandName
                            } with subcommand ${interaction.options.getSubcommand()}!`
                        )
                        .setTimestamp(),
                ],
            });
        ...
        if (
            subcommand.command !== interaction.commandName ||
            subcommand.subcommand !== interaction.options.getSubcommand()
        )
            return;
