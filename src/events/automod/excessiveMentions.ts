...
export default {
  ...
  async execute({ client, color }: EventArgs, message: Message) {
    ...
    if (config.excessiveMentions.alert) {
      const alertMessage = await message.channel.send({
        embeds: [
          new Embed(color)
            .setDescription(
              `Hey ${message.author}, please do not mention more than ${config.excessiveMentions.mentions} channels, users or roles! Your message has been deleted.`,
            )
            .setFooter({ text: `This message will be deleted in ${config.excessiveMentions.deleteAlertAfter} seconds.` }),
        ],
        content: `<@${message.author.id}>`,
      });
      setTimeout(() => {
        if (alertMessage.deletable) alertMessage.delete();
      }, config.excessiveMentions.deleteAlertAfter * 1000);
    }
    ...
    if (logChannel && config.logsEnabled) {
      ...
      const mentionStrikes = await AutomodStrike.countDocuments({
        guildId: message.guildId,
        userId: message.author.id,
        type: 'excessive-mentions',
      });
      logChannel.send({
        embeds: [
          new Embed(color)
            .setAuthor({ name: 'Excessive Mentions Message Deleted' })
            .setDescription(
              [
                ...
                `**Total Excessive Mentions Strikes**: ${mentionStrikes}`,
