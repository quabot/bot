...
class CustomEmbed extends EmbedBuilder {
  constructor(rawEmbed, getParsedString) {
    ...
    if (rawEmbed.title)
      embed.setTitle(`${getParsedString(rawEmbed.title)}`.substring(0, 256));
    ...
    if (rawEmbed.footer) {
      ...
      if (rawEmbed.footer.text)
        text = getParsedString(rawEmbed.footer.text).substring(0, 2048);
      if (
        rawEmbed.footer.icon &&
        isValidHttpUrl(getParsedString(rawEmbed.footer.icon).substring(0, 2048))
      )
    ...
    if (rawEmbed.author) {
      ...
      if (rawEmbed.author.text)
        name = getParsedString(rawEmbed.author.text).substring(0, 256);
      if (
        rawEmbed.author.url &&
        isValidHttpUrl(getParsedString(rawEmbed.author.url).substring(0, 2048))
      )
        url = getParsedString(rawEmbed.author.url).substring(0, 2048);
      if (
        rawEmbed.author.icon &&
        isValidHttpUrl(getParsedString(rawEmbed.author.icon).substring(0, 2048))
      )
    ...
    if (rawEmbed.description)
      embed.setDescription(
        getParsedString(rawEmbed.description).substring(0, 4096),
      );

    if (
      rawEmbed.url &&
      isValidHttpUrl(getParsedString(rawEmbed.url).substring(0, 2048))
    )
      embed.setURL(getParsedString(rawEmbed.url).substring(0, 2048));

    if (
      rawEmbed.thumbnail &&
      isValidHttpUrl(getParsedString(rawEmbed.thumbnail).substring(0, 2048))
    )
      embed.setThumbnail(
        getParsedString(rawEmbed.thumbnail).substring(0, 2048),
      );

    if (
      rawEmbed.image &&
      isValidHttpUrl(getParsedString(rawEmbed.image).substring(0, 2048))
    )
      embed.setImage(getParsedString(rawEmbed.image).substring(0, 2048));

    if (rawEmbed.color)
      embed.setColor(getParsedString(rawEmbed.color) ?? "#416683");
