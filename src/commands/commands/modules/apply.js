...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('id').setRequired(true).setDescription('The id of the application to apply for.')
        ),
    async execute(client, interaction, color) {
        ...
        if (!applicationConfig)
            return interaction
                .reply({
                    embeds: [
                        await generateEmbed(color, 'We just generated a new config. Please run the command again.'),
                    ],
                })
                .catch(e => {});

        if (applicationConfig.applicationEnabled === false)
            return interaction
                .reply({
        ...
        if (answer && application.applicationReapply === false)
            interaction
                .reply({
        ...
        if (
            application.requiredPermission !== 'none' &&
            !interaction.member.permissions.has(application.requiredPermission)
        )
            return interaction
                .reply({
                    embeds: [
                        await generateEmbed(
                            color,
                            "You don't have the required permissions to apply for that application."
                        ),
                    ],
                })
                .catch(e => {});
        ...
        if (answers.find(item => item.question === currentIndexId))
            currentAnswer = answers.find(item => item.question === currentIndexId);
        ...
        collector.on('collect', async interaction => {
            ...
            if (interaction.customId === enterId) {
                ...
                interaction.awaitModalSubmit({ filter, time: 600000 }).then(async interaction => {
                        ...
                        .update({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .addFields({
                                        ...
                                        value: `${
                                            q.question
                                        }\n\n**Your Answer**\n${interaction.fields.getTextInputValue('answer')}`.slice(
                                            0,
                                            1024
                                        ),
                                    }),
                            ...
                            components: [
                                new ActionRowBuilder({
                                    components: [
                                        currentAnswer ? editButton : enterButton,
                                        ...(currentIndex + 1 === application.applicationItems.length
                                            ? [submitButton]
                                            : []),
                                        ...(currentIndex ? [backButton] : []),
                                        ...(currentIndex + 1 < application.applicationItems.length
                ...
                const value = answers.find(item => item.question === currentIndex)
                    ? answers.find(item => item.question === currentIndex).value
                    : '';
            ...
            if (interaction.customId === submitId) {
                if (answers.length !== application.applicationItems.length)
                    return interaction
                        .reply({
                            embeds: [
                                await generateEmbed(
                                    color,
                                    `Please fill out all the questions before submitting. (${answers.length}/${application.applicationItems.length})`
                                ),
                            ],
                            ephemeral: true,
                        })
                ...
                await interaction
                    .update({
                        components: [new ActionRowBuilder().addComponents(editButton, submitButton, backButton)],
                    })
                    .catch(e => {});
                ...
                await interaction
                    .followUp({
                        embeds: [await generateEmbed(color, 'Successfully submitted your application.')],
                        ephemeral: true,
                    })
                    .catch(e => {});

                const logChannel = interaction.guild.channels.cache.get(applicationConfig.applicationLogChannelId);
                if (logChannel && applicationConfig.applicationLogEnabled)
                    logChannel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setColor(color)
