...
module.exports = {
    data: new SlashCommandBuilder().setName('quiz').setDescription('Play a quiz.').setDMPermission(false),
    /**
    ...
    async execute(client, interaction, color) {
        ...
        answerList.forEach(q => {
            ...
            answerButtons.addComponents(
                new ButtonBuilder().setCustomId(`${index}`).setLabel(q).setStyle(ButtonStyle.Primary)
            );
            disButtons.addComponents(
                new ButtonBuilder().setCustomId(`${index}`).setDisabled(true).setLabel(q).setStyle(style)
            );
            index++;
        });
        ...
        const message = await interaction
            .editReply({
                embeds: [new EmbedBuilder().setDescription(`${data[0].question}`).setColor(color)],
                fetchReply: true,
        ...
        collector.on('collect', async i => {
            ...
            const correct =
                answerList[i.customId - 1] === data[0].correctAnswer
                    ? 'You were **✅ correct**'
                    : 'You were **❌ incorrect**';

            await i.deferReply().catch(e => {});

            if (answerList[i.customId - 1] !== data[0].correctAnswer)
                disButtons.components[i.customId - 1].setStyle(ButtonStyle.Danger);
            ...
            i.editReply({
                embeds: [new EmbedBuilder().setColor(color).setDescription(`${correct}! Do you want to play again?`)],
                components: [
