...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('duration').setDescription('How long to timeout. (1h, 30min, 1w)').setRequired(true)
        )
        .addStringOption(option =>
        ...
        .addBooleanOption(option =>
            option
                .setName('private')
                .setDescription('Do you want the punishment to be only visible to you?.')
                .setRequired(false)
        )
    ...
    async execute(client, interaction, color) {
        ...
        const reason = interaction.options.getString('reason')
            ? interaction.options.getString('reason').slice(0, 800)
            : 'No reason specified.';
        let didTimeout = true;
        ...
        if (!user || !reason) {
            ...
            return interaction
                .editReply({
                    embeds: [
                        await generateEmbed(color, 'Please enter both a user and the reason for timing out this user.'),
                    ],
                })
                .catch(e => {});
        }
                    ...
                    .editReply({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    `QuaBot does not have permission to timeout that user - try moving the QuaBot role above all others`
                                )
                                .setColor(color),
                        ],
                        ephemeral,
            ...
            member
                .send({
                    embeds: [
                        await generateEmbed(
                            color,
                            `You were timed out on **${interaction.guild.name}**
