import { TextInputStyle, TextInputBuilder, Client, CommandInteraction, SlashCommandBuilder, ModalBuilder, ActionRowBuilder, ModalActionRowComponentBuilder } from "discord.js";
...
module.exports = {
    ...
    async execute(client: Client, interaction: CommandInteraction, color: any) {
        ...
        if (!suggestConfig) return await interaction.reply({
            embeds: [
                embed(color)
        ...
        if (!suggestConfig.enabled) return await interaction.reply({
            embeds: [
                embed(color)
        ...
        if (!suggestChannel) return await interaction.reply({
            embeds: [
                embed(color)
