const { Interaction, Client, PermissionFlagsBits, ActionRowBuilder, ButtonStyle, ButtonBuilder, EmbedBuilder } = require('discord.js');
...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (checkChannel(channel.type) === false) return interaction.editReply({
            embeds: [await generateEmbed(color, "Please enter a valid channel.")]
        }).catch((e => { }));

        if (winners > 25 || winners < 1) return interaction.editReply({
            embeds: [await generateEmbed(color, "Please enter between 1-25 winners.")]
        }).catch((e => { }));

        if (!ms(duration)) return interaction.editReply({
        ...
        if (giveawayConfig) { giveawayConfig.giveawayID = giveawayId; giveawayConfig.save(); }
        ...
        interaction.editReply({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(`The giveaway ends in <t:${endTimestamp}:R> and is hosted in ${channel}. [Jump to Message](${msg.url})`)
            ]
        }).catch((e => { }));
