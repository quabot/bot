...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (msg_id && !role && !channel)
            found = await ReactionRole.find({ guildId: interaction.guild.id, messageId: msg_id });
        if (msg_id && role && !channel)
            found = await ReactionRole.find({ guildId: interaction.guild.id, messageId: msg_id, roleId: role.id });
        if (msg_id && !role && channel)
            found = await ReactionRole.find({
                guildId: interaction.guild.id,
                messageId: msg_id,
        ...
        if (role && !msg_id && !channel)
            found = await ReactionRole.find({ guildId: interaction.guild.id, roleId: role.id });
        if (role && msg_id && !channel)
            found = await ReactionRole.find({ guildId: interaction.guild.id, roleId: role.id, messageId: msg_id });
        if (role && !msg_id && channel)
            found = await ReactionRole.find({ guildId: interaction.guild.id, roleId: role.id, channelId: channel.id });

        if (channel && !msg_id && !role)
            found = await ReactionRole.find({ guildId: interaction.guild.id, channelId: channel.id });
        if (channel && msg_id && !role)
            found = await ReactionRole.find({
                guildId: interaction.guild.id,
                channelId: channel.id,
        ...
        if (channel && !msg_id && role)
            found = await ReactionRole.find({ guildId: interaction.guild.id, channelId: channel.id, roleId: role.id });
                ...
                .editReply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(color)
                            .setDescription(
                                "Couldn't find any reactionroles that matched those criteria! Make sure you've got them [configured](https://dashboard.quabot.net)."
                            ),
                    ],
                    ephemeral: true,
                })
