...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (park) {
            ...
            const { data: rides } = await axios.get(`https://queue-times.com/parks/${id}/queue_times.json`).catch(() => { });
            if (rides.rides.length === 0) return interaction.reply({ embeds: [await generateEmbed(color, "No rides found for that park.\n[Powered by Queue-Times.com](https://queue-times.com)")]});
            ...
            const makeEmbed = async start => {
                const current = rides.rides.slice(start, start + 4);
                return new EmbedBuilder({
                    color: Colors.Aqua,
                    author: ({ name: "Powered by Queue-Times.com", url: "https://queue-times.com" }),
                    timestamp: Date.now(),
                    title: `Rides ${start + 1}${rides.rides.length > start + 4 ? "-" + (start + 4) : "-" + rides.rides.length}/${rides.rides.length} in ${park}`,
                    fields: await Promise.all(
                        current.map(async (item) => {
                            return ({
                                name: `${item.name}`,
            ...
            const canFit = rides.rides.length <= 4;
            const msg = await interaction.reply({
                embeds: [await makeEmbed(0)],
                ephemeral: false,
                components: canFit
                    ...
                    await interaction.update({
                        ...
                        components: [
                            new ActionRowBuilder({
                                components: [
                                    ...(currentIndex ? [backButton] : []),
                                    ...(currentIndex + 4 < rides.rides.length ? [forwardButton] : []),
                                ]
                            })
                        ]
                    }).catch((e => { }));
