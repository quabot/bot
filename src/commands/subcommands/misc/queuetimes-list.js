...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        const country = interaction.options.getString("country") ? titleCase(await interaction.options.getString("country") ? interaction.options.getString("country") : "") : null;
        ...
        const owner = interaction.options.getString("owner") ? titleCase(await interaction.options.getString("owner") ? interaction.options.getString("owner") : "") : null;
        ...
        if (country) {
            ...
            if (newParks.length === 0) return interaction.reply({ embeds: [await generateEmbed(color, "Couldn't find any rides for that country.\n[Powered by Queue-Times.com](https://queue-times.com)")] });
        ...
        if (park) {
            ...
            const makeEmbed = async start => {
                ...
                return new EmbedBuilder({
                    ...
                    author: ({ name: "Powered by Queue-Times.com", url: "https://queue-times.com" }),
                    timestamp: Date.now(),
                    title: `Rides ${start + 1}${rides.length > start + 4 ? "-" + (start + 4) : "-" + rides.length}/${rides.length} in ${park}`,
                    fields: await Promise.all(
                        current.map(async (item) => {
                            return ({
                                name: `${item.name}`,
        ...
        if (owner) {
            const newParks = [];
            parks.forEach(park => {
                if (park.name === owner) park.parks.forEach(subpark => {
                    newParks.push(subpark);
                });
            });

            if (newParks.length === 0) return interaction.reply({ embeds: [await generateEmbed(color, "Couldn't find any rides with that owner.\n[Powered by Queue-Times.com](https://queue-times.com)")] });
            ...
            const makeEmbed = async start => {
                ...
                return new EmbedBuilder({
                    ...
                    author: ({ name: "Powered by Queue-Times.com", url: "https://queue-times.com" }),
                    timestamp: Date.now(),
                    title: `Park(s) ${start + 1}${newParks.length > 4 ? "-" + (start + 4) : ""}/${newParks.length} owned by ${owner}`,
                    fields: await Promise.all(
                        current.map(async (item) => {
                            return ({
                                name: `${item.name}`,
