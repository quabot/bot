...
module.exports = {
    ...
    async execute(client, interaction, color) {
        const park = interaction.options.getString('park')
            ...
            : null;
        const attraction = interaction.options.getString('attraction')
            ? titleCase(
                  (await interaction.options.getString('attraction')) ? interaction.options.getString('attraction') : ''
              )
            : null;
            ...
            if (!foundAttraction)
                return interaction.reply({
                    embeds: [
                        await generateEmbed(
                            color,
                            "Couldn't find that ride! Get a list of rides for that park with `/queue-times list`.\n[Powered by Queue-Times.com](https://queue-times.com)"
                        ),
                    ],
                });
            ...
            interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            `${
                                foundAttraction.is_open
                                    ? `The queue is currently ${foundAttraction.wait_time} minutes.`
                                    : 'This ride is currently closed.'
                            }`
                        ),
                ],
            });
