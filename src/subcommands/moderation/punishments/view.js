...
module.exports = {
  ...
  async execute(client, interaction, color) {
    ...
    if (user)
      filtered = filtered.filter((punishment) => punishment.userId === user.id);
    if (staff)
      filtered = filtered.filter(
        (punishment) => punishment.moderatorId === staff.id,
      );
    if (type)
      filtered = filtered.filter((punishment) => punishment.type === type);
    if (id) filtered = filtered.filter((punishment) => punishment.id === id);
    if (userId)
      filtered = filtered.filter((punishment) => punishment.userId === userId);

    if (filtered.length === 0)
      return await interaction.editReply({
        embeds: [
          new Embed(color).setDescription(
            "No punishments with those filters found.",
          ),
        ],
      });
    ...
    const makeEmbed = async (start) => {
      ...
      return new EmbedBuilder({
        title: `Punishments ${start + 1}-${start + current.length}/${
          filtered.length
        }`,
        color: Colors.Red,
