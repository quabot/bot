...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (ticket.closed) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('This ticket is closed, reopen it to change the topic.')
			]
		});
		...
		if (!valid) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('You are not allowed to change the ticket topic.')
			]
		});
		...
		await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription(`Changed the topic to \`${newTopic}\`.`)
			]
		});     
	}
};
