...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (!valid) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('You are not allowed to transfer the ticket ownership.')
			]
		});

		if (!ticket.users.includes(user.id)) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('Please add the user to the ticket before changing the ownership.')
			]
		});
				...
				embeds: [
					new Embed(color)
						...
						.addFields(
							...
							{ name: 'Claimed By', value: `${ticket.staff === 'none' ? 'Unclaimed' : `<@${ticket.staff}>`}`, inline: true }
						)
				],
				components: [
		...
		await interaction
			.editReply({
				embeds: [
					new Embed(color)
						.setDescription(`Made ${user} the ticket owner.`)
				],
			});
	}
};
