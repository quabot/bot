...
module.exports = {
	...
	async execute(client, interaction, color) {
		...
		if (day === undefined || month === undefined || year === undefined) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('Please fill out all the fields.')
			]
		});
		if (day < 0 || day > 31 || month < 0 || month > 12 || year < 1900 || year > new Date().getFullYear()) return await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('Please enter a valid birthday.')
			]
		});
		...
		await interaction.editReply({
			embeds: [
				new Embed(color)
					.setDescription('Updated your birthday!')
			]
		});
	},
};
