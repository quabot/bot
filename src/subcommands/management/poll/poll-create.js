	...
	async execute(client, interaction, color) {
		...
		collector.on('collect', async i => {
			if (i.customId === 'details-poll') {
				...
				if (!document) return await i.reply({
					embeds: [
						new Embed(color)
				...
				if (!document.topic || !document.description || document.options.length === 0) return await i.reply({
					embeds: [
						new Embed(color)
							.setDescription('Please fill out all the required fields.')
					], ephemeral: true
				...
				const msg = await ch.send({ embeds: [embed], content: document.role ? `${document.role}` : null });
				if (!msg) return await i.reply({
					embeds: [
						new Embed(color)
				...
				await i.followUp({
					embeds: [
						new Embed(color)
							.setDescription(`The poll has started in ${ch}! [Click here](${msg.url}) to jump to the message.`)
					], ephemeral: true
				});
