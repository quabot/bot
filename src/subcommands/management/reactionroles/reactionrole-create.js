...
module.exports = {
    ...
    async execute(client, interaction, color) {
        ...
        if (!channel || !messageId || !role || !emoji || !mode || !requiredPermission) return await interaction.editReply({
            embeds: [
                new Embed(color)
        ...
        if (role.rawPosition > interaction.guild.members.me.roles.highest.rawPosition)
            return await interaction
                .editReply({
                    embeds: [
                        new Embed(color)
                            .setDescription('I cannot give that role to users! Make sure the role is below my roles.')
                    ],
                });
        ...
        if (await Reaction.findOne({ guildId: interaction.guildId, emoji: emoji, messageId }))
            return await interaction
                .editReply({
                    embeds: [
                        new Embed(color)
                            .setDescription('That emoji is already used for a reactionrole on that message.')
                    ]
                });

        channel.messages.fetch({ message: messageId }).then(async message => {

            if (!message) return await interaction.editReply({
                embeds: [
                    new Embed(color)
                        .setDescription(`Couldn't find any messages with that id in ${channel}.`)
                ]
            });

            await message.react(`${emoji}`).then(async () => {
                        ...
                        new Embed(color)
                            ...
                            .addFields(
                                ...
                                { name: 'Mode', value: `${mode}`, inline: true },
                                {
                                    ...
                                    value: `${requiredPermission === 'None' ? 'none' : await permissionBitToString(requiredPermission)}`,
                                    inline: true,
                                }
                            ),
                    ],
                ...
                return await interaction.editReply({
                    embeds: [
                        new Embed(color)
                            .setDescription(`That is not a valid emoji.`)
                    ]
                });
            });
