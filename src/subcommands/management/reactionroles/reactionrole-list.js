...
module.exports = {
  ...
  async execute(client, interaction, color) {
    ...
    if (role && !messageId && !channel) found = await Reaction.find({ guildId: interaction.guild.id, roleId: role.id });
    if (role && messageId && !channel)
      found = await Reaction.find({ guildId: interaction.guild.id, roleId: role.id, messageId: messageId });
    if (role && !messageId && channel)
