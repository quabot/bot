...
module.exports = {
    ...
    async execute(interaction, client, color) {
        ...
        const msg = await interaction.reply({
            embeds: [
                new MessageEmbed()
                    .setDescription(`**Mention** the new main log channel within 15 seconds.`)
                    .setColor(color)
                ...
                new MessageActionRow({
                    components: [
                        new MessageButton({
                            style: 'DANGER',
                            label: 'Cancel',
                            customId: "logging_channels_main_cancel"
                        }),
                    ]
                })
            ]
        ...
        collector.on('collect', async m => {
            ...
            const updated = new MessageEmbed()
                .setDescription(`Succesfully changed the main log channel to ${channel}`)
                .setColor(color)
        ...
        collectorCancel.on('collect', async interaction => {
            if (interaction.customId === "logging_channels_main_cancel") {
