...
module.exports = {
    ...
    async execute(interaction, client, color) {
        ...
        const msg = await interaction.reply({
            embeds: [
                new MessageEmbed()
                    .setDescription(`**Mention** the new level-up channel within 15 seconds.`)
                    .setColor(color)
                ...
                new MessageActionRow({
                    components: [
                        new MessageButton({
                            style: 'DANGER',
                            label: 'Cancel',
                            customId: "level_channels_set_cancel"
                        }),
                    ]
                })
            ]
        ...
        collector.on('collect', async m => {
            ...
            await guildDatabase.updateOne({
                levelChannelID: channel
            });

            const updated = new MessageEmbed()
                .setDescription(`Succesfully changed the level-up channel to ${channel}`)
                .setColor(color)
        ...
        collectorCancel.on('collect', async interaction => {
            if (interaction.customId === "level_channels_set_cancel") {
