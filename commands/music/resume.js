...
module.exports = {
    name: "resume",
    description: "Resume the music stream.",
    async execute(client, interaction) {
        ...
        try {
            ...
            const resumedEmbed = new MessageEmbed()
                .setTitle("▶️ Resumed the music stream!")
                .setColor(COLOR_MAIN)
            interaction.reply({ embeds: [resumedEmbed], components: [skipButtons] }).catch(err => console.log("Error!"));
        } catch (e) {
            interaction.channel.send({ embeds: [error] }).catch(err => console.log("Error!"));
