const discord = require('discord.js');
const DisTube = require('distube');

const Guild = require('../models/guild');
const colors = require('../files/colors.json');
const notVC = new discord.MessageEmbed()
    .setDescription("You need to be in a voice channel to play songs!")
    .setColor(colors.COLOR);
const musicOff = new discord.MessageEmbed()
    .setColor(colors.COLOR)
    .setDescription("Music is disabled!");
    const errorMain = new discord.MessageEmbed()
    .setDescription("There was an error!")
    .setColor(colors.COLOR)
const addedDatabase = new discord.MessageEmbed()
    .setDescription("This server is now added to our database.")
    .setColor(colors.COLOR)
module.exports = {
    name: "repeat",
    aliases: ["loop"],
    async execute(client, message, args) {

        console.log("Command `repeat` was used.");

        if (message.guild.me.hasPermission("MANAGE_MESSAGES")) message.delete({ timeout: 5000 });
        if (!message.guild.me.hasPermission("SEND_MESSAGES")) return;

        const settings = await Guild.findOne({
            guildID: message.guild.id
        }, (err, guild) => {
            if (err) message.channel.send(errorMain);
            if (!guild) {
                const newGuild = new Guild({
                    _id: mongoose.Types.ObjectID(),
                    guildID: message.guild.id,
                    guildName: message.guild.name,
                    prefix: config.PREFIX,
                    logChannelID: none,
                    enableLog: false,
                    enableSwearFilter: true,
                    enableMusic: true,
                    enableLevel: true,
                    mutedRoleName: muted,
                    mainRoleName: member
                });

                newGuild.save()
                    .catch(err => message.channel.send(errorMain));

                return message.channel.send(addedDatabase);
            }
        });

        if (settings.enableMusic === "false") return message.channel.send(musicOff);
        if (!message.member.voice.channel) return message.channel.send(notVC);

        const queue = client.player.getQueue(message);

        const noArgs1 = new discord.MessageEmbed()
            .setTitle("Repeat")
            .setDescription("Please enter either: `0 - OFF, 1 - REPEAT SONG or 2 - REPEAT QUEUE`")
            .setColor(colors.COLOR);
        if (!queue) return message.channel.send(noArgs1);

        const noArgs = new discord.MessageEmbed()
            .setTitle(":repeat: Repeat")
            .setDescription("Please enter either: `0 - OFF, 1 - REPEAT SONG or 2 - REPEAT QUEUE`")
            .addField("Current repeat mode", `\`${queue.repeatMode ? queue.repeatMode === 2 ? "All Queue :repeat:" : "This Song :repeat_one:" : "Off :arrow_forward:"}\``)
            .setColor(colors.COLOR);
        if (!args[0]) return message.channel.send(noArgs);

        let mode = client.player.setRepeatMode(message, parseInt(args[0]));
        mode = mode ? mode == 2 ? "Repeat queue\` :repeat:" : "Repeat song\` :repeat_one:" : "Off\` :arrow_forward:";
        const embed = new discord.MessageEmbed()
            .setDescription("Set repeat mode to `" + mode)
            .setColor(colors.COLOR);
        message.channel.send(embed);

    }
}