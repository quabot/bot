            ...
            switch (Sub) {
                case "message": {
                    ...
                    if (channel.type !== "GUILD_TEXT") {
                        const invalidText = new MessageEmbed()
                            ...
                            .setColor(COLOR_MAIN)
                        interaction.reply({ ephemeral: true, embeds: [invalidText] }).catch(err => console.log("Error!"));
                        return;
                    }
                    const message = new MessageEmbed()
                        ...
                        .setColor(COLOR_MAIN)
                        .setTimestamp()
                    interaction.reply({ embeds: [message] }).catch(err => console.log("Error!"));
                    ...
                    collector.on('collect', async m => {
                        if (m) {
                            ...
                            const messageSentTooLong = new MessageEmbed()
                                ...
                                .setColor(COLOR_MAIN)
                            if (m.content.length > 2000) return m.channel.send({ embeds: [messageSentTooLong] }).catch(err => console.log("Error!"));
                            channel.send(D)
                            const messageSent = new MessageEmbed()
                                .setDescription(`Your message has been sent to ${channel}.`)
                                .setColor(COLOR_MAIN)
                            m.reply({ embeds: [messageSent], allowedMentions: {repliedUser: false} }).catch(err => console.log("Error!"));
                            collector.stop();
                            return;
                        } else {
                            if (m.author.bot) return;
                            m.reply({ embeds: [timedOut], allowedMentions: {repliedUser: false} }).catch(err => console.log("Error!"));
                        }
                    });
                    break;
                }
                case "embed": {
                    ...
                    const messageSent = new MessageEmbed()
                        ...
                        .setColor(COLOR_MAIN)
                    interaction.reply({ embeds: [messageSent] }).catch(err => console.log("Error!"));
                    channel.send({ embeds: [embed] }).catch(err => console.log("Error!"));
                    break;
                }
            }
        } catch (e) {
