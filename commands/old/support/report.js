...
module.exports = {
    ...
    async execute(client, interaction) {
        try {
            ...
            if (guildDatabase.reportEnabled === "false") return interaction.reply({ embeds: [reportDis] }).catch(( err => { } ))

            const reportsChannel = interaction.guild.channels.cache.get(guildDatabase.reportChannelID);
            if (!reportsChannel) return interaction.reply({ embeds: [reportNoChannel] }).catch(( err => { } ))

            const content = interaction.options.getString('report');
            if (!content) return interaction.reply({ embeds: [reportNoReport] }).catch(( err => { } ))
            ...
            interaction.reply({ ephemeral: true, embeds: [reportSucces] }).catch(( err => { } ))
            ...
            reportsChannel.send({ embeds: [embed] }).catch(( err => { } ))
