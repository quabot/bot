    ...
    async execute(client, interaction) {
        ...
        try {
            ...
            if (suggestion) {
                ...
                client.guilds.cache.get('847828281860423690').channels.cache.get('940913767242403890').send({ embeds: [new MessageEmbed().setDescription(`${suggestion}`).setColor(COLOR_MAIN).setFooter(`Suggestion`)] }).catch(( err => { } ))
            }
            if (report) {
                const embed = new MessageEmbed()
                    .setColor(`GREEN`)
                ...
                interaction.reply({ embeds: [embed] }).catch(( err => { } ))
                client.guilds.cache.get('847828281860423690').channels.cache.get('940914011271204895').send({ embeds: [new MessageEmbed().setDescription(`${report}`).setColor(COLOR_MAIN).setFooter(`Bug Report`)] }).catch(( err => { } ))
            }
