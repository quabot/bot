...
module.exports = {
    ...
    async execute(client, interaction) {
        try {
            interaction.reply({ embeds: [memeScan] }).catch(err => console.log(err));
            ...
            client.guilds.cache.get('847828281860423690').channels.cache.get('938509157710061608').send({ embeds: [new MessageEmbed().setTitle(`Error!`).setDescription(`${e}`).setColor(`RED`).setFooter(`Command: meme`)] }).catch(err => console.log(err));;
            return;
        }
    }
}