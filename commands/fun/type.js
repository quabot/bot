        ...
        try {
            ...
            collector.on('collect', m => {
                ...
                if (m.content === random) {
                    ...
                    m.reply({ embeds: [embed], allowedMentions: { repliedUser: false } }).catch(err => console.log("Error!"));;
                    collector.stop();
                    return;
                } else {
                    const timeSpent = new Date().getTime() - startTime;
                    ...
                    m.reply({ embeds: [empty2], allowedMentions: { repliedUser: false } }).catch(err => console.log("Error!"));;
                    collector.stop();
                    return;
                }
            });
            ...
            client.guilds.cache.get('847828281860423690').channels.cache.get('938509157710061608').send({ embeds: [new MessageEmbed().setTitle(`Error!`).setDescription(`${e}`).setColor(`RED`).setFooter(`Command: type`)] }).catch(err => console.log("Error!"));;
            return;
        }
    }
}