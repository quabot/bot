...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        console.log("Command `seek` was used.");
        ...
        if (args[0] <= 0) return message.channel.send(noNumber);
        if (args[0] >= 6000) return message.channel.send(noNumber);
        if (isNaN(args[0])) return message.channel.send(noNumber);

        message.channel.send();
        client.player.seek(message, Number(args[0] * 1000));

        const seekedTo = new discord.MessageEmbed()
            .setDescription(`Seeked song to \`${args[0]}\`!`)
            .setColor(colors.COLOR);
        message.channel.send(seekedTo);
    }
}